...
export let options: Options = {
  ...
  thresholds: {
 //   http_req_duration: ["p(90) < 500", "p(95) < 800", "p(99) < 1000", "med<600"], // med/avg of requests must complete below 1.5s
...
export default (cookies: string[]) => {
 ...
 // const cookie = randomUser;
  ...
  const workflowQuery = WORKFLOW_STATUS_QUERY;
  const res = http.post(
    authoringApiUrl,
  ...
  check(res, {
    "Workflow Status-API response is 200": () => result === 200,
  });
};

export function handleSummary(data: any) {
    return {
      "./results/workflowStatusApi.html": htmlReport(data),
